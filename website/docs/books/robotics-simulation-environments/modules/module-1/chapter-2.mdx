...
```
C₁ = (p₂ - p₁) · a = 0  (axis alignment)
C₂ = (p₂ - p₁) · b = 0  (position constraint)
```

Where p₁, p₂ are attachment points, a, b are constraint axes.
...
**Constraint Equations**:
```
C₁ = (v₂ - v₁) · a = 0  (velocity constraint)
C₂ = (p₂ - p₁) · b = 0  (position constraint)
```

Where v₁, v₂ are linear velocities.
...
**Pressure Force**:
```
F_pressure = -mᵢ * Σⱼ (Pᵢ/ρᵢ² + Pⱼ/ρⱼ²) * ∇W(|rᵢ - rⱼ|, h)
```
